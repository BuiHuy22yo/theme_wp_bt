...
if ( ! function_exists( 'action_find_showroom' ) ) {
	function action_find_showroom() {
		$product_showroom = arrow_get_option( 'product_showroom', true );
		$enable_action    = arrow_get_value_in_array( $product_showroom, 'enable_action' );
		$title            = arrow_get_value_in_array( $product_showroom, 'title' );
...
if ( ! function_exists( 'get_product_policy' ) ) {
	function get_product_policy() {
		$product_policy = arrow_get_option( 'product_policy' );
		$html           = array();
		...
		foreach ( $product_policy as $policy ) {
			$title       = arrow_get_value_in_array( $policy, 'title' );
			$description = arrow_get_value_in_array( $policy, 'description' );
...
if ( ! function_exists( 'get_product_tabs_summary' ) ) {
	function get_product_tabs_summary() {
		...
		$specifications_img  = arrow_get_value_in_array( $specifications_img, 'url' );
...
//		}
		...
		if ( '' !== get_post()->post_content ) {
			$html[] = '<li class="item active"><a href="#tsb1">' . esc_html__( 'Product Description', 'arrow' ) . '</a></li>';
		}
//		if ( $specifications_list || $specifications_img ) {
		$html[] = '<li class="item"><a href="#tsb1">' . esc_html__( 'Detail', 'arrow' ) . '</a></li>';
//		}
		$html[] = '</ul>';
		...
		if ( $specifications_list || $specifications_img ) {
			...
			foreach ( $specifications_list as $specifications ) {
				$title       = arrow_get_value_in_array( $specifications, 'title' );
				$description = arrow_get_value_in_array( $specifications, 'description' );
...
if ( ! function_exists( 'get_product_tabs_policy' ) ) {
	function get_product_tabs_policy() {
		$product_policy_tab_1 = arrow_get_option( 'product_policy_tab_1' );
		$product_policy_tab_2 = arrow_get_option( 'product_policy_tab_2' );
		$tab1_title           = arrow_get_value_in_array( $product_policy_tab_1, 'title' );
		$tab1_content         = arrow_get_value_in_array( $product_policy_tab_1, 'content' );
		$tab2_title           = arrow_get_value_in_array( $product_policy_tab_2, 'title' );
		$tab2_list_item       = arrow_get_value_in_array( $product_policy_tab_2, 'list_item' );
		$active_t1            = $active_t2 = '';
		$html                 = array();
		...
		if ( $tab2_title && $tab2_list_item ) {
			...
			foreach ( $tab2_list_item as $item ) {
				$title = arrow_get_value_in_array( $item, 'title' );
				$url   = arrow_get_value_in_array( $item, 'url' );
...
if ( ! function_exists( 'buy_now' ) ) {
    function buy_now() {

            echo '<span>';
            echo  arrow_get_option('button_buy_before','demo');
            echo  get_woocommerce_currency_symbol();
            echo  get_post_meta( get_the_ID(), '_regular_price',true);
            echo  '</span>';
            echo  arrow_get_option('button_buy','demo');

    }
}

if ( ! function_exists( 'purchase_now' ) ) {
    function purchase_now() {

            return arrow_get_option('button_purchase_now','demo');
